...
| 7 | Hungarian is O(n³) per layer — width 1024? | **partly answered (W11)** — the practical answer may be to stop aligning: a G-invariant equivariant reader (W11b) needs no template and no assignment at all, and reaches f = 0.526 against c_align's 0.628. The amortized/Sinkhorn path is still owed for settings that need an explicit frame. | (amortized/learned canonicalization + Sinkhorn path; honest scaling section) |
