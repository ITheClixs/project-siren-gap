...
**Purpose.** A review pass asked, correctly, that the boundary between what this program
...
**Note on review contributions.** Four components here exist because a review of an earlier draft
...
reader was named in that review. Ours is the two-layer realization.
...
| M7 | Bias phasors $(\cos b, \sin b)$ as the route to a $D_\infty$-aware reader | **suggested in review**, implemented here | The review named the route explicitly: "replace raw bias $b$ by the phasor $(\cos b,\sin b)$, which removes the integer $2\pi$ translation. The remaining sign and reflection actions can then be represented by finite-dimensional linear transformations." Sign-equivariant weight-space layers are prior work (Monomial-NFN, ScaleGMN) | the **idea is not ours**. Ours is the two-layer realization: the $\Z_2\times\Z_2$ character grading, the identification of which message channels are character-legal once $W^2$ is graded $(1,1)$ on one side and $(1,0)$ on the other, and the verification that the resulting reader is invariant for unbounded windings (T16 at $\|j\|\le40$; audited on fitted corpora at $3.3\times10^{-6}$) |
...
| E3 | The orbit-only intervention (S6) | **ours**, prompted by review | none known | holding each fitted network and its function fixed while randomizing the group, so the measured degradation has one cause |
| E4 | The matched non-invariant control (S7) | **ours**, prompted by review | none known | a feature map matched to W10 in monomial degree, trigonometric order, pooling and dimension, differing only in whether the $D_\infty$ component is quotiented out |
