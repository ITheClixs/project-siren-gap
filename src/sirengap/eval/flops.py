...
def weight_phasor_reader(
    ...
    bilinear mixes per layer, 4 graded linear maps per layer and 4 message projections, all d x d
    over every node, against the graph reader's two. At width 186 that is ~146 MFLOP/INR against
    ~54, and the accounting says so rather than flattering the construction. The count equals
    PyTorch's FlopCounterMode on the actual forward pass (test T14); an earlier version
    double-counted the graded maps and omitted the message projections (~163 MFLOP).

    Costs, per INR:

      features    the phasor lift, the two Gram diagonals and E^2 -- O(w^2) and O(w * in_dim)
      embed       four graded linear maps per layer, fan-in the block widths (m + 7 and c + 7)
      per round   4 message contractions ([p,n] x [n,d]) and their 4 projections, then 10
                  bilinear mixes and 4 graded linear maps on each layer's w nodes
    ...
    embed = MAC * w * d * ((m + 7) + (c + 7))           # layer-1 and layer-2 block widths
    per_round = (
        4 * MAC * w * w * d                  # the four message contractions against E and E^2
        + 4 * MAC * w * d * d                # the four message projections, one per contraction
        + 2 * 10 * MAC * w * d * d           # graded bilinear: 10 character pairs, both layers
        + 2 * n_blocks * MAC * w * d * d     # graded linear mix: one map per block, both layers
    )
